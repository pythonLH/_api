...
from api_util.mex_util.mex_api import queryStageRepayLoanLists
...
class Test_queryStageAuditLoanLists:
    @pytest.mark.parametrize('loans_list', america_parma['LoanLists'])
    def test_queryStageAuditLoanLists(self, loans_list):
        Log().debug(f"测试:{loans_list['_caseTitle']}")

        _result = queryStageRepayLoanLists(_param=json.dumps(loans_list['_caseParam']))
